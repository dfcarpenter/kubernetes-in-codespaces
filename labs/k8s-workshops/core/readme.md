...
**NOTE:** If you are coming from the [cli tutorial](../kubecli/readme.md), you may have completed this already.
...
3) Create a new context called `k3ddev` within the `k3d-k3s-default` cluster  as the `admin@k3d-k3s-default` user, with the namespace
 set to `dev`.
```
$ kubectl config set-context k3ddev --cluster=k3d-k3s-default --user=admin@k3d-k3s-default --namespace=dev
```

4) Switch to the newly created context.
```
$ kubectl config use-context k3ddev
```
